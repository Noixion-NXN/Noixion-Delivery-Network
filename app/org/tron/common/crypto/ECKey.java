...
public class ECKey implements Serializable {
  ...
  // The two parts of the kad_key. If "priv" is set, "pub" can always be
  ...
  /**
   * Generate a new keypair using the given Java Security Provider. <p> All private kad_key operations
  ...
  /**
   * Pair a private kad_key with a public EC point. <p> All private kad_key operations will use the
   * provider.
   */
  public ECKey(Provider provider, @Nullable PrivateKey privKey, ECPoint pub) {
    ...
    if (privKey == null || isECPrivateKey(privKey)) {
      ...
      throw new IllegalArgumentException(
          "Expected EC private kad_key, given a private kad_key object with" +
              " class " +
    ...
    if (pub == null) {
      throw new IllegalArgumentException("Public kad_key may not be null");
    } else {
      this.pub = pub;
    }
  }
  ...
  /**
   * Pair a private kad_key integer with a public EC point <p> BouncyCastle will be used as the Java
   * Security Provider
   */
  ...
  /* Test if a generic private kad_key is an EC private kad_key
   *
   ...
   * a fallback that covers this case is to check the kad_key algorithm
   */
  ...
  private static PrivateKey privateKeyFromBigInteger(BigInteger priv) {
    if (priv == null) {
      ...
      try {
        ...
        throw new AssertionError("Assumed correct kad_key spec statically");
      }
    }
  }
  ...
  /**
   * Creates an ECKey given the private kad_key only.
   *
   * @param privKey -
   * @return -
   */
   ...
   * multiplying the generator point by the private kad_key. This is used to speed things up when you
  ...
  public static ECKey fromPrivateAndPrecalculatedPublic(byte[] priv, byte[]
      pub) {
    check(priv != null, "Private kad_key must not be null");
    check(pub != null, "Public kad_key must not be null");
  ...
  /**
   * Returns public kad_key bytes from the given private kad_key. To convert a byte array into a BigInteger,
  ...
  /**
   * Compute an address from an encoded public kad_key.
   *
   * @param pubBytes an encoded (uncompressed) public kad_key
  ...
  /**
   * Compute the encoded X, Y coordinates of a public point. <p> This is the encoded public kad_key
  ...
  /**
   * Recover the public kad_key from an encoded node id.
   *
  ...
  public static byte[] signatureToKeyBytes(byte[] messageHash,
    ...
    check(messageHash.length == 32, "messageHash argument has length " +
        messageHash.length);
    int header = sig.v;
    // The header byte: 0x1B = first kad_key with even y, 0x1C = first kad_key
    // with odd y,
    //                  0x1D = second kad_key with even y, 0x1E = second kad_key
    // with odd y
    ...
    byte[] key = ECKey.recoverPubBytesFromSignature(recId, sig,
        messageHash);
    if (key == null) {
      throw new SignatureException("Could not recover public kad_key from " +
          "signature");
    }
    return key;
  }
  ...
  /**
   * Compute the address of the kad_key that signed the given signature.
   *
  ...
  /**
   * Compute the kad_key that signed the given signature.
   *
   * @param messageHash 32-byte hash of message
   * @param signatureBase64 Base-64 encoded signature
   * @return ECKey
  ...
  /**
   * <p>Verifies the given ECDSA signature against the message bytes using the public kad_key bytes.</p>
   * <p> <p>When using native ECDSA verification, data must be 32 bytes, and no element may be
   * larger than 520 bytes.</p>
   *
   * @param data Hash of the data to verify.
   * @param signature signature.
   * @param pub The public kad_key bytes to use.
   * @return -
   */
  ...
  /**
   * Verifies the given ASN.1 encoded ECDSA signature against a hash using the public kad_key.
   *
   * @param data Hash of the data to verify.
   * @param signature signature.
   * @param pub The public kad_key bytes to use.
  ...
  /**
   * <p>Given the components of a signature and a selector value, recover and return the public kad_key
   ...
   * one. Because the kad_key recovery operation yields multiple potential allKeys, the correct kad_key must
   * either be stored alongside the signature, or you must be willing to try each recId in turn
   * until you find one that outputs the kad_key you are expecting.</p> <p> <p>If this method returns
   ...
   * output is null OR a kad_key that is not the one you expect, you try again with the next recId.</p>
   *
   * @param recId Which possible kad_key to recover.
   * @param sig the R and S components of the signature, wrapped.
   * @param messageHash Hash of the data that was signed.
   * @return 65-byte encoded public kad_key
   */
  @Nullable
  public static byte[] recoverPubBytesFromSignature(int recId,
    ...
    //   1.6.1. Compute a candidate public kad_key as:
    //               Q = mi(r) * (sR - eG)
    //
  ...
  /**
   * Decompress a compressed public kad_key (x co-ord and low-bit of y-coord).
   *
   * @param xBN -
   * @param yBit -
   * @return -
  ...
  /**
   * Returns a copy of this kad_key, but with the public point represented in uncompressed form.
  ...
  /**
   * Returns true if this kad_key doesn't have access to private kad_key bytes. This may be because it was
   * never given any private kad_key bytes to begin with (a watching kad_key).
   *
   * @return -
   */
  ...
  /**
   * Returns true if this kad_key has access to private kad_key bytes. Does the opposite of {@link
   * #isPubKeyOnly()}.
   *
   * @return -
   */
  ...
  /**
   * Gets the address form of the public kad_key.
   *
   * @return 20-byte address
   */
  ...
  /**
   * Generates the NodeID based on this kad_key, that is the public kad_key without first format byte
   */
  ...
  /**
   * Gets the encoded public kad_key value.
   *
   * @return 65-byte encoded public kad_key
   */
  ...
  /**
   * Gets the public kad_key in the form of an elliptic curve point object from Bouncy Castle.
   *
   * @return -
   */
  ...
  /**
   * Gets the private kad_key in the form of an integer field element. The public kad_key is derived by
   * performing EC point addition this number of times (i.e. point multiplying).
   *
   * @return -
   * @throws IllegalStateException if the private kad_key bytes are not available.
   */
  ...
  /**
   * Returns whether this kad_key is using the compressed form or not. Compressed pubkeys are only 33
   * bytes, not 64.
   *
   * @return -
   */
  ...
  /**
   * Produce a string rendering of the ECKey INCLUDING the private kad_key. Unless you absolutely need
   * the private kad_key it is better for security reasons to just use toString().
   *
   * @return -
   */
  ...
  public ECDSASignature doSign(byte[] input) {
    ...
    // No decryption of private kad_key required.
    if (privKey == null) {
      throw new MissingPrivateKeyException();
    }
  ...
  public ECDSASignature sign(byte[] messageHash) {
    ...
    if (recId == -1) {
      throw new RuntimeException("Could not construct a recoverable kad_key" +
  ...
  public BigInteger keyAgreement(ECPoint otherParty) {
    if (privKey == null) {
      ...
      try {
        ...
        throw new RuntimeException("ECDH kad_key agreement failure", ex);
      }
    }
  }
   ...
   * @deprecated should not use EC private scalar value as an AES kad_key
   */
  ...
  /**
   * Verifies the given R/S pair (signature) against a hash using the public kad_key.
   *
   * @param sigHash -
   * @param signature -
   * @return -
  ...
  /**
   * Returns a 32 byte array containing the private kad_key, or null if the kad_key is encrypted or public
   * only
   *
   * @return -
   */
